...
void XXX::Do(MainWindow* mw) {
	...
	tx=x+(urand()%3)-1;
	ty=y+(urand()%3)-1;
	GameObject* go=mw->Get(tx, ty);
	if(go!=nullptr){
		if(go->Type==TXXX){
			XXX* xd=(XXX*)go;
			for(int i=0;i<NetSize*NetSize;i++){
				net[i]=(net[i]+((xd->net)[i]))/2;
			}
		}
	}
...
void XXX::chk(int ax, int ay, MainWindow* mw) {
...
void XXX::SetNew(int ax, int ay, MainWindow* mw) {
	if (energy!=0) {
		if ((urand() % energy) > net[5+RNDOF]) {
			XXX* go = mw->p.GXXX(ax, ay);
			go->Mutable(net);
